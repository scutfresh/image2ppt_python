...
# === 1. 基础几何容器 ===
        ...
        "rightTriangle": MSO_AUTO_SHAPE_TYPE.RIGHT_TRIANGLE,
        ...
        "pentagon": MSO_AUTO_SHAPE_TYPE.REGULAR_PENTAGON,
        "hexagon": MSO_AUTO_SHAPE_TYPE.HEXAGON,
        "octagon": MSO_AUTO_SHAPE_TYPE.OCTAGON,
        "cross": MSO_AUTO_SHAPE_TYPE.CROSS,
        "teardrop": MSO_AUTO_SHAPE_TYPE.TEARDROP,
    
        # === 3. 现代 UI 卡片变体 ===
        "snipRoundRect": MSO_AUTO_SHAPE_TYPE.SNIP_1_RECT,  
        "frame": MSO_AUTO_SHAPE_TYPE.FRAME,                
        "halfFrame": MSO_AUTO_SHAPE_TYPE.HALF_FRAME,       
    
        # === 4. 块状逻辑箭头与循环 ===
        "rightArrow": MSO_AUTO_SHAPE_TYPE.RIGHT_ARROW,
        "leftArrow": MSO_AUTO_SHAPE_TYPE.LEFT_ARROW,
        "upArrow": MSO_AUTO_SHAPE_TYPE.UP_ARROW,
        "downArrow": MSO_AUTO_SHAPE_TYPE.DOWN_ARROW,
        "leftRightArrow": MSO_AUTO_SHAPE_TYPE.LEFT_RIGHT_ARROW,
        "chevron": MSO_AUTO_SHAPE_TYPE.CHEVRON,            
        "bentArrow": MSO_AUTO_SHAPE_TYPE.BENT_ARROW,
        "uTurnArrow": MSO_AUTO_SHAPE_TYPE.U_TURN_ARROW,
        "circularArrow": MSO_AUTO_SHAPE_TYPE.CIRCULAR_ARROW,
    
        # === 5. 特殊标注与强调 ===
        "bubble": MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGULAR_CALLOUT, 
        "cloudCallout": MSO_AUTO_SHAPE_TYPE.CLOUD_CALLOUT,         
        "ribbon": MSO_AUTO_SHAPE_TYPE.UP_RIBBON,                   
        "star": MSO_AUTO_SHAPE_TYPE.STAR_5_POINT,
    
        # === 6. 符号与运算符 ===
        "mathPlus": MSO_AUTO_SHAPE_TYPE.MATH_PLUS,
        "mathEqual": MSO_AUTO_SHAPE_TYPE.MATH_EQUAL,
        "mathMultiply": MSO_AUTO_SHAPE_TYPE.MATH_MULTIPLY,
    }
...
def normalize_slides(manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    elif isinstance(manifest.get("slide"), dict):
        single = manifest["slide"]
        slides = [{**single, "name": single.get("name") or manifest.get("name") or "Slide 1"}]
    else:
...
def normalize_element_coords(element: dict[str, Any]) -> dict[str, Any]:
    """如果元素有 bbox 字段，将其内容提升到顶层。"""
    if "bbox" in element and isinstance(element["bbox"], dict):
        bbox = element["bbox"]
        element.setdefault("x", bbox.get("x"))
...
def build_pptx(manifest_path: Path, output_path: Path | None = None, summary_path: Path | None = None) -> dict[str, Any]:
    ...
    for slide_data in slides_data:
        ...
        for element in elements:
            element = normalize_element_coords(element)
