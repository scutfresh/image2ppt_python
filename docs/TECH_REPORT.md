# Image2PPT Langchain Workflow - Technical Report
...
Vision:
- IMAGE2PPT_VISION_MODEL (默认 qwen-vl)
- IMAGE2PPT_VISION_BASE_URL
- IMAGE2PPT_VISION_API_KEY (或 DASHSCOPE_API_KEY / OPENAI_API_KEY)
- IMAGE2PPT_VISION_TEMPERATURE (默认 0.2)

ImageGen:
- IMAGE2PPT_IMAGEGEN_MODEL (默认 z-image-turbo)
- IMAGE2PPT_IMAGEGEN_BASE_URL
- IMAGE2PPT_IMAGEGEN_API_KEY (或 DASHSCOPE_API_KEY / OPENAI_API_KEY)
- IMAGE2PPT_IMAGEGEN_SIZE (默认 1024x1024)
- IMAGE2PPT_IMAGEGEN_BACKGROUND (可选)
- --imagegen-api-style: openai 或 qwen

运行示例与参数说明见 [langchain/README.md](langchain/README.md)。
...
限制与假设:
- 目前面向位图输入 (PNG/JPEG/WebP)。
- 单个 PPTX 内所有幻灯片必须使用一致尺寸。
- LLM 输出需可修复为 JSON，严重破损仍会失败。

## 10. 扩展点

- 可通过扩展 [langchain/imagegen.py](langchain/imagegen.py) 的 `request_image` 与 `extract_image_bytes` 适配新供应商。
- 可在 [langchain/image2pptx.py](langchain/image2pptx.py) 的 `add_element` 中扩展新元素类型。
- Prompt 版本统一集中在 [langchain/prompts.py](langchain/prompts.py)，便于规范升级。

## 11. 批量评测与运维辅助

- [langchain/test.py](langchain/test.py) 提供多模型批量测试流程，可用于评估不同视觉模型输出质量。
- [langchain/rename_vision_reports.py](langchain/rename_vision_reports.py) 用于统一历史报告命名，便于整理。

## 12. 依赖

核心依赖为 python-pptx、Pillow、requests，详见 [langchain/requirements.txt](langchain/requirements.txt)。
