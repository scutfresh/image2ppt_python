...
The image content is attached. Analyze the slide deeply and output a JSON object. 

CRITICAL COORDINATE RULE:
ALL `bbox` fields MUST use a 0-1000 relative scale (where 0 is top/left, 1000 is bottom/right). 
Example: A box covering the entire image is {{"x": 0, "y": 0, "w": 1000, "h": 1000}}. 
NEVER use values greater than 1000.

Output the exact following JSON structure:

- background: object with type (color|gradient|image), color/gradient/image hints and bbox (x, y, w, h) in 0-1000 scale.
- titles: list of text blocks with text, bbox in 0-1000 scale, font_family, font_size_px, color, bold, italic, align
...
- objects: list of visual objects with:
  ...
  bbox (x, y, w, h) in 0-1000 scale,
  z_index (integer),
  render_method (MUST BE EITHER "svg" OR "crop"),
  needs_transparent (true/false),
  style_tags (string, ONLY if render_method is "svg")
- shapes: list of simple shapes (rect|roundRect|ellipse|line) with bbox in 0-1000 scale, fill, stroke, stroke_width_px
